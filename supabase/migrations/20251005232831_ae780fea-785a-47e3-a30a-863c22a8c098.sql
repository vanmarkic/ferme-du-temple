...
-- All submissions must now go through the rate-limited edge function

-- Drop the dangerous public insert policy that allowed unrestricted data insertion
DROP POLICY IF EXISTS "Allow public inserts" ON public.inscriptions;

-- Add documentation comments to clarify the security model
...
-- Document the inscription_attempts table security model
